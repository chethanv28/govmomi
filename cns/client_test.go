...
// TestBatchRelocateVolume creates N block volumes (default 2, controlled by
// CNS_BATCH_RELOCATE_COUNT), relocates all of them in a single
// cnsClient.RelocateVolume call, and prints which volumes succeeded and which
// failed.
//
// Required env vars:
//
//	CNS_VC_URL              – vCenter SOAP URL
//	CNS_DATACENTER          – datacenter name
//	CNS_DATASTORE           – source datastore
//	CNS_MIGRATION_DATASTORE – destination datastore for the relocation
//
// Optional env var:
//
//	CNS_BATCH_RELOCATE_COUNT – number of volumes to create and relocate (≥2, default 2)
func TestBatchRelocateVolume(t *testing.T) {
	ctx := context.Background()
	...
	if url == "" || datacenter == "" || datastore == "" {
		t.Skip("CNS_VC_URL, CNS_DATACENTER, or CNS_DATASTORE is not set")
	}
	if datastoreForMigration == "" {
		t.Skip("CNS_MIGRATION_DATASTORE is not set; skipping batch relocate test")
	}

	volumeCount := 2
	if countStr := os.Getenv("CNS_BATCH_RELOCATE_COUNT"); countStr != "" {
		if n, err := strconv.Atoi(countStr); err == nil && n >= 2 {
			volumeCount = n
		}
	}
	...
	if cnsClient.Version == ReleaseVSAN67u3 || cnsClient.Version == ReleaseVSAN70 {
		t.Skipf("RelocateVolume API is not supported on vSAN release %q", cnsClient.Version)
	}
	...
	dsList := []vim25types.ManagedObjectReference{ds.Reference()}

	// Create volumeCount volumes and register cleanup to delete them all.
	volumeIDs := make([]string, 0, volumeCount)
	t.Cleanup(func() {
		if len(volumeIDs) == 0 {
			return
		}
		var toDelete []cnstypes.CnsVolumeId
		for _, id := range volumeIDs {
			toDelete = append(toDelete, cnstypes.CnsVolumeId{Id: id})
		}
		t.Logf("Cleanup: deleting %d volume(s): %v", len(toDelete), volumeIDs)
		deleteTask, err := cnsClient.DeleteVolume(ctx, toDelete, true)
		if err != nil {
			t.Logf("Cleanup: DeleteVolume call failed: %+v", err)
			return
		}
		deleteTaskInfo, err := GetTaskInfo(ctx, deleteTask)
		if err != nil {
			t.Logf("Cleanup: failed to get delete task info: %+v", err)
			return
		}
		results, err := GetTaskResultArray(ctx, deleteTaskInfo)
		if err != nil {
			t.Logf("Cleanup: failed to get delete task results: %+v", err)
			return
		}
		for _, r := range results {
			res := r.GetCnsVolumeOperationResult()
			if res.Fault != nil {
				t.Logf("Cleanup: failed to delete volume %q: fault=%+v", res.VolumeId.Id, res.Fault)
			} else {
				t.Logf("Cleanup: deleted volume %q", res.VolumeId.Id)
			}
		}
	})

	for i := 0; i < volumeCount; i++ {
		volumeName := fmt.Sprintf("batch-relocate-pvc-%s", uuid.New().String())
		...
		createTask, err := cnsClient.CreateVolume(ctx, []cnstypes.CnsVolumeCreateSpec{createSpec})
		if err != nil {
			t.Fatalf("volume %d: CreateVolume call failed: %+v", i+1, err)
		}
		createTaskInfo, err := GetTaskInfo(ctx, createTask)
		if err != nil {
			t.Fatalf("volume %d: GetTaskInfo failed: %+v", i+1, err)
		}
		createTaskResult, err := GetTaskResult(ctx, createTaskInfo)
		if err != nil {
			t.Fatalf("volume %d: GetTaskResult failed: %+v", i+1, err)
		}
		if createTaskResult == nil {
			t.Fatalf("volume %d: empty create task result", i+1)
		}
		createOpRes := createTaskResult.GetCnsVolumeOperationResult()
		if createOpRes.Fault != nil {
			t.Fatalf("volume %d: CreateVolume fault: %+v", i+1, createOpRes.Fault)
		}
		volID := createOpRes.VolumeId.Id
		t.Logf("Created volume %d/%d: ID=%s name=%s", i+1, volumeCount, volID, volumeName)
		volumeIDs = append(volumeIDs, volID)
	}

	// Build a single batch relocate call with all volume specs.
	relocateSpecs := make([]cnstypes.BaseCnsVolumeRelocateSpec, 0, len(volumeIDs))
	for _, volID := range volumeIDs {
		relocateSpecs = append(relocateSpecs, cnstypes.CnsBlockVolumeRelocateSpec{
			CnsVolumeRelocateSpec: cnstypes.CnsVolumeRelocateSpec{
				VolumeId:  cnstypes.CnsVolumeId{Id: volID},
				Datastore: migrationDS.Reference(),
			},
		})
	}

	t.Logf("Issuing batch RelocateVolume for %d volumes → datastore %q", len(relocateSpecs), datastoreForMigration)
	relocateTask, err := cnsClient.RelocateVolume(ctx, relocateSpecs...)
	if err != nil {
		t.Fatalf("RelocateVolume batch call failed: %+v", err)
	}

	relocateTaskInfo, err := GetTaskInfo(ctx, relocateTask)
	if err != nil {
		t.Fatalf("GetTaskInfo for RelocateVolume failed: %+v", err)
	}

	taskResults, err := GetTaskResultArray(ctx, relocateTaskInfo)
	if err != nil {
		t.Fatalf("GetTaskResultArray for RelocateVolume failed: %+v", err)
	}

	var succeeded, failed []string
	for _, result := range taskResults {
		res := result.GetCnsVolumeOperationResult()
		volID := res.VolumeId.Id
		if res.Fault != nil {
			t.Logf("  [FAILED]    volume %q — fault: %+v", volID, res.Fault)
			failed = append(failed, volID)
		} else {
			t.Logf("  [SUCCEEDED] volume %q", volID)
			succeeded = append(succeeded, volID)
		}
	}

	t.Logf("Batch relocate summary: %d/%d succeeded, %d/%d failed",
		len(succeeded), len(taskResults), len(failed), len(taskResults))
	if len(succeeded) > 0 {
		t.Logf("Succeeded: %v", succeeded)
	}
	if len(failed) > 0 {
		t.Logf("Failed:    %v", failed)
		t.Errorf("batch relocate: %d volume(s) failed to relocate", len(failed))
	}
}
